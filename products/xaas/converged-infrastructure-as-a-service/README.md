# Converged Infrastructure as a Service: From Capex Blueprint to Partner-Sellable Platform
 
> The engineering-level companion to the CIaaS case study: how the reference architecture, the consumption model and the partner go-to-market behind it were actually built.
 
**Type:** case study
**Status:** live, fully scoped with partners in pilot at departure
...
What existed instead was a legacy converged infrastructure blueprint that was not fit for purpose as a service. Capex only, with no consumption model behind it. No partner portal, so nothing for a partner to quote, order or transact through. No managed support wrapped around it, which meant that even where a customer wanted to consume rather than own, the operational responsibility still landed back on them.
...
The obvious approach would have been to build the whole service and launch it once. I rejected that early. I defined CIaaS instead as a stack of layers, each one sellable on its own and each one adding value to the layer beneath it, so revenue could start before the full service existed and each layer could be validated with real partners before the next was committed to.
 
The layers ran: infrastructure first, then support and managed services, then the hypervisor layer, then observability, then the application layer. That order was not arbitrary. Infrastructure alone was the minimum a partner could resell. Support and managed services was the layer that actually removed operational responsibility from the customer, which was the reason most of them were interested in the first place. The hypervisor layer moved the service up from raw capacity into something a customer could run workloads on directly, sequenced with VMware vSphere first and OpenShift second, since vSphere was the platform the existing customer base already ran and OpenShift was the container play behind it. Observability made the estate manageable at scale and fed the managed services layer above it.
 
The application layer was deliberately left undefined. It would have been custom professional services and managed services work per customer, and nothing about it could carry fixed pricing. Defining it properly would have meant either pretending it was standardisable when it was not, or holding the whole roadmap up waiting for an answer. It stayed on the roadmap as a named future layer with the honest caveat attached, rather than being scoped into something it could not be. The demand signal behind it was real, since customers had expressed a clear desire for 360 support across their entire stack rather than the infrastructure alone, but that signal was never taken further to test whether it was commercially viable. It was a placeholder with a reason, not a plan.
...
I documented all of it in a 38-page PRD covering high level, mid level and low level design as a single collective document, linked out to the related documentation around it. It was deliberately detailed. A product built out of four other teams' components, sold through partners, and priced off a consumption model only works if every party involved is reading the same requirements, and the PRD is what made that possible rather than relying on individual conversations to keep everyone aligned.
 
Because it had to serve executives, engineers, partners and the commercial teams at the same time, it carried the whole product rather than just the technical design:
 
- Executive summary, and the business reasoning and research behind the case for building it at all
- Go-to-market strategy and the use cases the service was being built to serve
- Target operating model, covering how the service would actually be run once live
- Consumption costs and the commercial model underneath them
- Integration with the partner portal for quote-to-cash and automated procurement
- T-shirt sizing down to node level, with CPU, RAM and storage detail per node
...
Both target buyers needed tenant separation, the partner reselling to many end customers and the enterprise serving many internal business units, so multi-tenancy was a hard requirement for CIaaS to work at all.
 
It did not need designing. The underlying converged infrastructure stack CIaaS was built on already had a reference architecture in place that handled it: mostly VLANs and VXLANs, covering the majority of deployments, with NSX-based software-defined networking available as the separation layer where VMware Cloud Foundation was in play, giving micro-segmentation and tenant isolation natively in the virtualisation layer.
 
That mattered to the product decision rather than the engineering one. Tenant separation was already solved, which meant my work here was validating that the existing approach held up against both buyer profiles and the t-shirt sizes, not building a separation model from scratch.
...
A CPQ tool generated the builds and returned the underlying cost base. Everything on top of that was product work. I set the margin, cost structure, discount tiering and final pricing, then loaded them into the partner quote-to-cash portal as SKU-type entries, by t-shirt size and by add-on. That translation step is what turned an internal configuration tool into something a partner could sell from without needing the vendor to price every deal.
...
The immediate return was visibility into the other teams' roadmaps, which meant I could extend my own roadmap further out with confidence, planning against what was actually coming rather than against what had already shipped.
...
The business only had access to perpetual licensing, on terms that materially limited what could be offered and supported as a subscription service. A consumption product cannot be built on perpetual licensing without either breaking the commercial model or breaking the terms.
 
This was before Broadcom acquired VMware, and at that point the only subscription route available was one where the customer brought their own licensing. That looked like a solution and was not. If the customer holds the licences, the service provider cannot cleanly support or manage the platform on their behalf, which undermines the exact layer, support and managed services, that made CIaaS worth buying.
 
What the product actually needed was subscription licensing the business could resell in its own right, and that did not exist on the terms available. Getting there was a split effort. I drove and owned the requirement internally, defining what the licensing model had to do for the product to work, and the partner manager took it to the vendor to negotiate a procurement model that fitted. We then widened the conversation to bring in other teams across the business who would benefit from the same licensing structure, so the case for better terms carried more weight than one product's requirement would have on its own. What I pushed for was the subscription tiers themselves, cost levels that made the layer viable inside the CIaaS price points, and minimum commit levels the consumption model could actually work with.
 
That negotiation was in play and approaching sign-off when I left. The layer itself was already sized and priced, with sales and marketing material drafted and ready for the marketing team to brand and format.
...
[Bare Metal as a Service](../bare-metal-as-a-service/README.md) is covered in full in its own entry, but the way it got approved belongs here, because it was a CIaaS roadmap item and a genuine internal obstacle.
...
NetFoundry started as [Network as a Service](../network-as-a-service/README.md) and is covered in its own entry. Its origin sits with CIaaS.
...
- 38-page PRD covering high, mid and low level design as a single collective document, spanning executive summary, business reasoning and research, go-to-market strategy, use cases, target operating model, consumption costs, partner portal quote-to-cash and automated procurement integration, node-level t-shirt sizing with CPU, RAM and storage per node, networking and architecture diagrams, and the layered roadmap including Bare Metal as a Service
...
- Subscription licensing requirement for the hypervisor layer, owned and driven internally with the partner manager taking it to the vendor, and widened to other teams who would benefit from the same terms
...
- Took a net-new consumption service from a legacy Capex-only blueprint to a validated, partner-sellable platform, becoming the main value driver in the XaaS portfolio
...
By the end of this role the service was fully scoped, with the PRDs created and validated, the reference architectures, consumption model and pricing in place, and active partners in pilot stage.
...
The work ended before the product matured, following a group reorganisation that moved the XaaS products into another team.
