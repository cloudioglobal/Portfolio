...
That mattered to the product decision rather than the engineering one. Tenant separation was already solved, which meant the work here was validating that the existing approach held up against both buyer profiles and the t-shirt sizes, not building a separation model from scratch.
...
This was before Broadcom acquired VMware, and at that point the only subscription route available was one where the customer brought their own licensing. That looked like a solution and wasn't. If the customer holds the licences, the service provider cannot cleanly support or manage the platform on their behalf, which undermines the exact layer, support and managed services, that made CIaaS worth buying. What the product actually needed was subscription licensing the business could resell in its own right, and that did not exist on the terms available. Getting there was a split effort. This role drove and owned the requirement internally, defining what the licensing model had to do for the product to work, and the partner manager took it to the vendor to negotiate a procurement model that fitted. The conversation was then widened to bring in other teams across the business who would benefit from the same licensing structure, so the case for better terms carried more weight than one product's requirement would have on its own. The requirements pushed for were the subscription tiers themselves, cost levels that made the layer viable inside the CIaaS price points, and minimum commit levels the consumption model could actually work with.
...
Bare Metal as a Service is covered in full in its own entry ([Bare Metal as a Service with Canonical MaaS](../bare-metal-as-a-service/README.md)), but the way it got approved belongs here, because it was a CIaaS roadmap item and a genuine internal obstacle.
